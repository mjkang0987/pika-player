...
기획서 S1~S7 은 모두 화면과 동작이 붙었습니다. 남은 것은 다듬는 쪽입니다.

- **검색은 보관함에만 있습니다.** 폴더·최근 탭에는 없습니다. 필요하면 같은 상태를
  각 화면으로 옮기면 됩니다
- **폴더 화면의 정렬은 영상에만 적용됩니다.** 폴더는 늘 이름순입니다
- **SAF 폴더는 영상 개수·용량을 보여주지 않습니다.** 세려면 하위를 전부 열어봐야 해서
  폴더 목록을 띄우는 것만으로 느려집니다
